...
def parse_display_date(value: object, field_name: str) -> tuple[str, str]:
...
def validate_plan_payload(
    *,
    ...
    plan_display, plan_iso = parse_display_date(plan_date, "Plan Date")
    verify_display, verify_iso = parse_display_date(verify_date, "Verify Date")
...
def effective_check_status(plan: dict, *, today_iso: str | None = None) -> str:
    ...
    verify_iso = normalize_text(plan.get("verify_date_iso"))
    if verify_iso and verify_iso <= ref:
        return "miss"
    return "pending"
...
PLANNING_AUDIT_ACTIONS = {
    "created": "Created",
    "deleted": "Deleted",
    "check_confirmed": "Confirmed delivery",
    "prepared": "Prepared",
}
...
def format_audit_log_line(entry: dict) -> str:
    ...
    return f"{when} · {action} · {dg} · {item} · {supplier} · Plan {plan_date} · by {actor}"
