...
Ví dụ:
  python tools/add_user.py admin secret123 "Nguyễn Văn A"
"""
...
def main() -> None:
    if len(sys.argv) < 3:
        print("Usage: python tools/add_user.py <username> <password> [display_name]")
        sys.exit(1)
    username = sys.argv[1]
    password = sys.argv[2]
    ...
    if db.get_user_by_username(username):
        ...
        sys.exit(1)
    uid = db.create_user(username, password, display)
